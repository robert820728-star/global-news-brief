...
class MaterializeSourceScansTests(unittest.TestCase):
    def test_discovery_priority_is_not_public_value_v2(self):
        signals = MODULE.discovery_signals(
        ...
        self.assertTrue(signals["policy"])
        self.assertTrue(signals["broad_scope"])
        self.assertGreater(MODULE.discovery_priority_score(signals), 0)
...
<pubDate>Sat, 15 Aug 2026 06:00:00 +0800</pubDate><description>Boundary witness.</description></item>
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("discovery_priority_score", ranked)
            self.assertIn("discovery_signals", ranked)
            self.assertNotIn("importance_score", ranked)
            self.assertNotIn("importance_breakdown", ranked)
