...
class ValidatorTests(unittest.TestCase):
    ...
    def test_images_reject_more_than_two_assets(self):
        ...
        second.update({
            ...
            "caption": "圖二：現場受損情況（來源：官方來源）。",
            "kind": "news_photo",
            "content_sha256": "b" * 64,
            "incremental_information": "補充官方資訊圖未呈現的現場受損情況。",
        })
        third = copy.deepcopy(second)
        third.update({
            "path": "sandbox:/tmp/image-3.png",
            "caption": "圖三：另一處現場受損情況（來源：官方來源）。",
            "content_sha256": "c" * 64,
        })
        ...
        self.assertTrue(any("超過每則 2 張圖片上限" in error for error in errors), errors)
    ...
    def test_images_reject_duplicate_content_hash(self):
        ...
        second.update({
            ...
            "caption": "圖二：重複內容（來源：官方來源）。",
            "incremental_information": "聲稱提供另一個角度。",
        })
        manifest["events"][0]["images"]["assets"].append(second)

        errors = VALIDATOR.validate_manifest_data(manifest)

        self.assertTrue(any("重複圖片內容 SHA-256" in error for error in errors), errors)
