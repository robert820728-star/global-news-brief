...
class MobileRunLogTests(unittest.TestCase):
    ...
    def prepare(self, run_id="run-001", at="2026-08-17T21:58:00Z"):
    ...
    def test_next_prepare_rotates_completed_current_to_previous(self):
        ...
        self.prepare(run_id="run-002", at="2026-08-18T21:58:00Z")
        self.assertEqual(self.read("previous.json")["run_id"], "run-001")
        self.assertEqual(self.read("previous.json")["status"], "completed")
        self.assertEqual(self.read("current.json")["run_id"], "run-002")
    ...
    def test_rotation_replaces_the_older_previous_record(self):
        self.prepare("run-001")
        self.prepare("run-002", "2026-08-18T21:58:00Z")
        self.prepare("run-003", "2026-08-19T21:58:00Z")
        self.assertEqual(self.read("previous.json")["run_id"], "run-002")
        self.assertNotIn("run-001", (self.ledger_dir / "previous.json").read_text(encoding="utf-8"))
        workflow = (ROOT / ".github" / "workflows" / "prepare-mobile-run-ledger.yml").read_text(
            encoding="utf-8"
        )
