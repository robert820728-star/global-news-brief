...
`VISIBLE_MEDIA_SCHEDULE_ELIGIBILITY_GATE` and `VISIBLE_LOCAL_ATTACHMENT_INSTALL_SMOKE_GATE` apply before this routing for every formal recurring schedule. The task is eligible only when it is bound to a ChatGPT desktop/Codex desktop local project or worktree, can run as `full-runtime`, and has already attached `maps/generated/taiwan-counties-yellow-v2.png` from the verified workspace as a local image that is actually visible in the current conversation. An external URL, Markdown hotlink, path string, caption, or broken placeholder does not pass the smoke test. If either gate is not satisfied, do not create or resume an occurrence and do not begin news discovery; reinstall or move the recurring task to an eligible local-project execution surface.
...
- If the local execution backend is absent, or the pinned loader seed cannot be obtained and the host has no lossless connector-to-local file handoff, classify the diagnostic as `host_execution_unavailable` or `bootstrap_seed_transport_unavailable`, not as a repository materialization defect. For a formal recurring schedule, stop before occurrence creation and before news discovery, and require reinstall on the eligible desktop/local-project surface. Do not modify `main` and do not create a doomed `mobile-native` run that waits for a nonexistent scheduled recovery worker.
- `mobile-chatgpt-daily-prompt.md` remains available only for an explicitly requested one-time diagnostic, candidate review, source check, or recovery of a historical mobile run. It is not the fallback producer for the formal recurring Reader and must not be used to claim that the production schedule is installed or canonically completed.
- The repository never prepares a future occurrence before the Scheduled Task actually fires. After the probe confirms the eligible `full-runtime` route, that executor creates/resumes the occurrence with immutable `execution_mode=full-runtime`. The next scheduled occurrence probes capabilities again and resolves fresh `main`; it must not create a second task merely to retry missing runtime.
