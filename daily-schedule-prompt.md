...
- `PENDING_CANDIDATE_AUDIT_WORK_IS_NOT_BLOCKER_GATE`: mobile-native 必須呼叫 `candidate_audit_status`，以 exact run 的 `candidate-audit-work/progress.json` 找到唯一 `next_operation`，每次等待 durable commit 後從**第一個未完成** review／score 單元接續。待處理 row／batch 數、已耗模型時間、context 大小或 token 顧慮只是 `in_progress`，不是不可恢復 blocker；不得因此輸出 failure receipt、重跑 discovery、建立 replacement run，並且**不得停用** exact task。只有具體 identity、hash、conservation、validator 或 transport failure 才可 fail-closed。
