...
- 每則 image evidence 必須保存 `original_source_attempted`、`official_fallback_attempted`、`wire_fallback_attempted`、`reliable_media_fallback_attempted`、`qualified_image_found`、`delivery_attempted` 與 `delivery_result`。任一來源層尚未實際搜尋時，不得宣告 fallback exhaustion、圖片不可取得或不可恢復 blocker；只要任一路徑找到合格圖片，就必須繼續嘗試該執行模式可用的實際可見交付。
...
- If the local execution backend is absent, or the pinned loader seed cannot be obtained and the host has no lossless connector-to-local file handoff, select `mobile-native` before creating the occurrence. Classify these as `host_execution_unavailable` or `bootstrap_seed_transport_unavailable`, not as a repository materialization defect. Do not disable the daily schedule and do not modify `main`.
...
- A `mobile-native` result is a canonical reader release only when no verified source image remains undelivered and the delivery path available to the selected execution mode has actually been attempted. Only after `IMAGE_FALLBACK_EXHAUSTION_GATE` has completed all four source tiers, at least one qualified image has been found, and every host-available visible-delivery method has actually failed may the run record `reader-canonical-capability-degraded`, `native_media_status=unavailable`, and `NATIVE_MEDIA_UNAVAILABLE`; keep the same run `status=running` at `visuals-completed`. It must not advance to reader delivery or `status=completed`. A later full-runtime visual-recovery executor resumes only the missing visual delivery and must not rerun the news stages; the original run keeps `execution_mode=mobile-native`, so this recovery is not a mode switch. True source exhaustion requires all four source tiers to have been searched and no qualified image found; only then may the image be omitted without that capability code.
...
11. pinned seed 或 direct loader transport 不可用時，只有宿主具備 lossless connector-to-local byte handoff 才可使用既有 segmented connector fallback。沒有該能力時，停止 full-runtime 嘗試並讓同一排程依前述 capability routing 走 `mobile-native`；不得記成 repository materialization failure、不得建立 replacement run。
...
9. `collect-news-images`
   - `IMAGE_FALLBACK_EXHAUSTION_GATE` applies before every image blocker: per event, actually search original source → official/party → original wire → other reliable same-event publication/reprint, and save the seven checklist fields. A single URL, site, download, card, or delivery failure is never exhaustion. Text verification and image provenance may use different reliable sources when the image is legal, traceable, and matches the same event/date/person/location.
