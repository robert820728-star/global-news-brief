...
`SCHEDULE_PROMPT_UPDATE_PRECEDES_SMOKE_GATE`, `SCHEDULE_PROMPT_CAPABILITY_AWARE_VERIFICATION_GATE`, and `SCHEDULE_PROMPT_EXACT_ID_READBACK_ONLY_GATE` apply before capability routing: installation or repair first validates and submits the newest complete `scheduled-task-prompt-template.md`. When the same control plane and namespace support exact-task-ID saved-prompt readback, the installer must query the ID returned by create/update and compare the complete text. Without exact-ID view, the pre-submit check proves the complete unabridged outbound payload, while the authoritative create/update result must prove the task ID, successful save/update, daily 06:00 schedule, timezone, and current-conversation destination. An empty generic list/search from an unknown scope cannot negate successful creation and cannot trigger a duplicate create; only an exact-ID view from the same control plane can do so. Only then does `SAME_SCHEDULED_HOST_VISIBLE_SCREENSHOT_SMOKE_GATE` require the same ChatGPT Scheduled Task host to show a native image card or directly screenshot a public page's image region. The smoke does not require a verified workspace, repository map, original download, or original quality. A URL, Markdown hotlink, path string, caption, or broken placeholder does not pass. Failure retains the newest prompt and pauses the task; it must never leave an old prompt enabled.

`INSTALL_CONTROL_PLANE_FAST_PATH` and `SINGLETON_SCHEDULE_INSTALL_GATE` require the installer to freeze `ensure_singleton` before touching the control plane. Before the first create/update it reads only the installation files, not news runtime documents. It then uses authoritative task inventory: one exact match is adopted and updated; authoritative absence permits one create; `multiple_present` or `unknown` forbids blind creation and completion claims. `update_existing` requires an exact task ID. An unknown first-create outcome records the actual control-plane error and operation identity and is reconciled without a second create.

`IMMUTABLE_INSTALL_MAIN_RESOLUTION_GATE` applies to the installer before this runtime contract is loaded: every paste-ready singleton starter resolves `main` through fresh nonce-bearing named-branch and commit endpoints, requires the same 40-character SHA, and reads `INSTALL.md`, the scheduled-task template, and install helpers only from that immutable SHA. A mutable `/main/` raw URL, search summary, cached model memory, or prior installation SHA is never install authority. After immutable contract and outbound-payload validation, the first control-plane operation is the authoritative inventory required by `SINGLETON_SCHEDULE_INSTALL_GATE`; news runtime work remains forbidden until one exact task ID is established.
