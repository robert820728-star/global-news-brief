...
`SCHEDULE_PROMPT_UPDATE_PRECEDES_SMOKE_GATE` and `SCHEDULE_PROMPT_CAPABILITY_AWARE_VERIFICATION_GATE` apply before capability routing: installation or repair first validates and submits the newest complete `scheduled-task-prompt-template.md`. When the control plane supports saved-prompt readback, it must reread and compare the complete text. When the control plane explicitly lacks readback, the pre-submit check proves the complete unabridged outbound payload, while the authoritative create/update result must prove the task ID, successful save/update, daily 06:00 schedule, timezone, and current-conversation destination; missing evidence fails, and the installer must not blindly create a duplicate task. Only then does `SAME_SCHEDULED_HOST_VISIBLE_SCREENSHOT_SMOKE_GATE` require the same ChatGPT Scheduled Task host to show a native image card or directly screenshot a public page's image region. The smoke does not require a verified workspace, repository map, original download, or original quality. A URL, Markdown hotlink, path string, caption, or broken placeholder does not pass. Failure retains the newest prompt and pauses the task; it must never leave an old prompt enabled.
