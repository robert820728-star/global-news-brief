...
- A discovery source failure must not block the whole brief. Continue in degraded mode when at least one configured discovery route yields verifiable current candidates. A same-source browser snapshot is the final route fallback; cross-source web search may find same-source leads or verification evidence but cannot inject canonical candidates or satisfy route coverage. Stop only if no configured-route candidate can be verified.
...
- A `mobile-native` result is a canonical reader release under its declared delivery profile. It may use `reader-canonical-capability-degraded` only after the delivery path available to the selected execution mode has actually been attempted: full-runtime uses download, screenshot fallback, materialization, and attachment delivery; mobile-native uses source inspection and the host's native image/card route. The run may still become `status=completed`; it records `native_media_status=unavailable` and `NATIVE_MEDIA_UNAVAILABLE`, preserves image evidence and omission notes, and must not claim unavailable local-file, attachment, or rendered-pixel validation. A later full-runtime pass may enrich only the missing visual delivery and must not rerun the news stages.
...
1. `source-scan`
   ...
   - materializer 必須為每條 configured discovery route 產生 source coverage row。`scan_status=completed` 只表示已取得頁面成功物化；coverage 是否完整另由 `coverage_complete`／`coverage_status` 決定。部分來源的已驗證列可繼續入池，失敗來源以 `scan_status=failed`、零 counts 與 `coverage_status=unavailable` 保留；不得在 candidate audit 或 release receipt 洗成完整 coverage。
