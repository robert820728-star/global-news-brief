...
`INSTALL_CONTROL_PLANE_FAST_PATH` and `EXPLICIT_SCHEDULE_INSTALL_INTENT_GATE` require the installer to freeze `create_new` or `update_existing` before touching the control plane. Before the first create/update it reads only the installation files, not news runtime documents. Explicit `create_new` performs exactly one create after outbound validation and does not require a same-title list/search first; `update_existing` requires an exact task ID. An unknown first-create outcome records the actual control-plane error and is reconciled without a second create.

`IMMUTABLE_INSTALL_MAIN_RESOLUTION_GATE` applies to the installer before this runtime contract is loaded: every paste-ready create starter resolves `main` through fresh nonce-bearing named-branch and commit endpoints, requires the same 40-character SHA, and reads `INSTALL.md`, the scheduled-task template, and install helpers only from that immutable SHA. A mutable `/main/` raw URL, search summary, cached model memory, or prior installation SHA is never install authority. For explicit `create_new`, the first control-plane operation after immutable contract and outbound-payload validation is the single create; list/search/inventory is forbidden before that create.
