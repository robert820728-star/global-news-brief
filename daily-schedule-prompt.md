...
`RECOVER_BEFORE_REFUSAL_GATE`：已授權 occurrence 的任何 validator、hash、identity、conservation、transport、持久化、render、receipt 或可見媒體失敗，都先轉成內部 `action=resume_required`，回到唯一 owning stage，用相同 run／SHA／window 重做失敗動作；保留仍有效的已完成 artifact。相同錯誤三次有界恢復耗盡前，不得向使用者輸出 blocker。只有外部權限、不可恢復 occurrence 身分、不可取得 immutable bytes 或持續不可用必要 transport 在三次後仍成立，才可回精簡 blocker receipt。validator 的非零結果是重做指令，不是整輪拒絕交付。

`EVERY_DAILY_NEWS_EXECUTION_GATE`

`VISIBLE_MEDIA_SCHEDULE_ELIGIBILITY_GATE`

Any daily-news invocation—manual, single-run, test, first-run, recurring, or resume—uses the same news and visible-image requirements. full-runtime may download or directly screenshot and materialize local attachments. A no-local-Python ChatGPT Scheduled Task may use its native image card or direct page/image-region screenshot path after the occurrence independently probes that route. Trigger type never relaxes visible attachment delivery, but original files and original quality are not required. Runtime visible-media eligibility does not depend on an installation smoke result.
...
`VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE`: a writable filesystem and executable runtime prove only `local_execution_capable`; they do not prove source-media acquisition or final delivery. Prefer the local attachment route only after separate probes establish `source_media_byte_fetch` (or an actually available `webpage_region_screenshot`), decode/hash validation, and `local_attachment_media_handoff`. If that complete pipeline is unavailable, use another independently verified delivery route such as `native_image_search`; never infer screenshot support from `page_open`. A serialized text ref still fails delivery.
...
- `GDELT_ARTICLE_HYDRATION_BOUNDARY`：GDELT 是 aggregator，candidate `canonical_url` 可指向外部原始發布站。其文章 hydration 允許 public HTTPS，但必須把該列 canonical hostname 固定為 verified article host；任何 fetch override 或 HTTP redirect 跨 host 都必須拒絕。CNA／中新社仍維持各自 configured source-domain boundary。
- A regional-supplement discovery failure does not block another covered section. When the run includes the fallback/global section, `GLOBAL_SECTION_PRIMARY_DISCOVERY_GATE` prefers a successful configured primary aggregator. After every configured primary route is exhausted, verified cross-source global results may be submitted to the run-bound bridge through `web_fallback_materialize` in batches of 1–20 and admitted as the reserved `web_fallback` coverage row. Each batch must preserve provider/query/evidence URL, primary-failure evidence, exact result rank, original article URL and in-window publication evidence in append-only receipts and replayable snapshots. That row must remain `coverage_complete=false` / `degraded_partial`, cannot satisfy configured-route completeness or impersonate GDELT, and its public-HTTPS article hydration must remain on the verified article host. If neither primary discovery nor verified `web_fallback` yields candidates, remain at source-scan even when regional candidates exist.
...
- `PENDING_CANDIDATE_AUDIT_WORK_IS_NOT_BLOCKER_GATE`: mobile-native 必須呼叫 `candidate_audit_status`，以 exact run 的 `candidate-audit-work/progress.json` 找到唯一 `next_operation`，每次等待 durable commit 後從**第一個未完成** review／score 單元接續。待處理 row／batch 數、已耗模型時間、context 大小或 token 顧慮只是 `in_progress`，不是不可恢復 blocker；不得因此輸出 failure receipt、重跑 discovery、建立 replacement run，並且**不得停用** exact task。具體 identity、hash、conservation、validator 或 transport failure 必須先回到 owning stage 重做失敗動作；三次有界恢復耗盡後，只有外部權限／身分／完整性仍無法成立才可回 blocker。
...
`USABLE_FIRST_SCHEDULE_INSTALL_GATE` and `POST_INSTALL_DIAGNOSTICS_GATE` apply before capability routing. Installation or repair first resolves its mutually exclusive entry state, then uses one canonical mutation to submit the complete `scheduled-task-prompt-template.md`, daily 06:00 schedule, account timezone, current-conversation delivery, and `enabled=true`. A successful control-plane response with the exact task ID acknowledges usable installation. Saved-prompt/timezone/next-run readback and `SAME_SCHEDULED_HOST_VISIBLE_SCREENSHOT_SMOKE_GATE` are post-install diagnostics; unavailable evidence is `verification_partial` and must not pause, disable, delete, recreate, or duplicate the formal task.

`SCHEDULE_PROMPT_UPDATE_PRECEDES_SMOKE_GATE`, `SCHEDULE_PROMPT_CAPABILITY_AWARE_VERIFICATION_GATE`, and `SCHEDULE_PROMPT_EXACT_ID_READBACK_ONLY_GATE` preserve canonical bytes and evidence honesty: validate the full outbound prompt before mutation; use exact-ID readback when available; never claim unavailable fields were verified. Generic list/search output cannot negate a successful exact-ID acknowledgement. Explicit content mismatch is repaired by idempotently resubmitting the canonical desired state to the same exact ID, never by creating another task.

`INSTALL_CONTROL_PLANE_FAST_PATH` and `SINGLETON_SCHEDULE_INSTALL_GATE` require the installer to freeze `ensure_singleton` and classify one mutually exclusive entry state. `new_without_exact_id` uses authoritative task inventory before any mutation; `known_exact_id_resume` idempotently submits desired state to that exact ID without another inventory; `create_outcome_unknown` reconciles the original operation identity without another create. Before the first mutation it reads only installation files, not news runtime documents. `multiple_present` or `unknown` forbids blind creation.

`IMMUTABLE_INSTALL_MAIN_RESOLUTION_GATE` applies to the installer before this runtime contract is loaded: every paste-ready singleton starter resolves `main` through fresh nonce-bearing named-branch and commit endpoints, requires the same 40-character SHA, and reads `INSTALL.md`, the scheduled-task template, and install helpers only from that immutable SHA. A mutable `/main/` raw URL, search summary, cached model memory, or prior installation SHA is never install authority. After immutable contract and outbound-payload validation, only `new_without_exact_id` starts with authoritative inventory; the two recovery states keep their existing identity. News runtime work remains forbidden until one exact task ID is established.

Before reading the recursive tree, capsule manifest, helpers, payload, or chunks, perform exactly one no-network capability probe using the host's native execution tool. Record its result only as `local_execution_capable`: a writable temporary workspace and a Python runtime able to execute a standard-library statement. Probe `bootstrap_transport_capable` separately by actually obtaining and verifying the pinned loader seed; record `verified_workspace_ready` only after the loader verifies the capsule and workspace receipt. No earlier result implies a later one. A shell or container command is not required when the host exposes Python through a native data-analysis tool.
...
- `BOOTSTRAP_TRANSPORT_DOWNGRADE_GATE`: if the local execution backend is absent, or the pinned loader seed cannot be obtained and the host has no lossless connector-to-local file handoff, classify the diagnostic as `host_execution_unavailable` or `bootstrap_seed_transport_unavailable`, not as a repository defect. If the same Scheduled Task host already passed at least one independent visible-media delivery route, route the same occurrence to `mobile-chatgpt-daily-prompt.md`; do not wait for a nonexistent future full-runtime worker.
...
11. `BOOTSTRAP_TRANSPORT_DOWNGRADE_GATE`：pinned seed 或 direct loader transport 不可用時，只有宿主具備 lossless connector-to-local byte handoff 才使用既有 segmented connector fallback。沒有該能力時，不得記成 repository materialization failure、不得建立 replacement run；若安裝 smoke 已證明同宿主至少一條可見媒體路徑，立即把 same occurrence 轉入 `mobile-native`，否則才在 occurrence 與 discovery 前停於 host capability blocker。
...
只有在 capability routing 後仍選定 full-runtime（only after full-runtime remains selected），且 capsule／receipt／runtime workspace acquisition 真正失敗時，Stage -1 blocker 才固定回報為：
...
3. `select-news-events`
   ...
   - 產生 `selection-results.json` 後，必須先執行 `scripts/validate_selection_freshness.py --selection <selection-results> --source-candidates <source-candidates>`。此 gate 必須確認每個事件 URL 都在本輪 fresh pool、所有 C 級以上候選都有有效 `selected_event_id`，且映射事件實際存在；首次失敗立即回到 `select-news-events`，用相同 fresh pool 重做完整選稿映射，不能刪單筆後重跑掩蓋，也不能直接輸出 blocker。
...
9. `collect-news-images`
   - `IMAGE_FALLBACK_EXHAUSTION_GATE` applies before every image blocker. The Scheduled Task resolves media across original source → official/party → original wire → local reliable media with a local-language query → other reliable same-event publication/reprint. It uses a validated local attachment only when the complete `VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE` is proven; otherwise it selects another independently verified native media route. A serialized ref fails immediately. A page-image screenshot is attempted only when `webpage_region_screenshot` independently passed, and `page_open` never implies that result. A bare URL is not delivery. Save `original_source_attempted`, `direct_media_url_attempted`, `official_fallback_attempted`, `wire_fallback_attempted`, `reliable_media_fallback_attempted`, `qualified_image_found`, `delivery_attempted`, and `delivery_result`. Text verification and image provenance may use different reliable sources when the image is legal, traceable, and matches the same event/date/person/location.
...
一般來源、圖片、地圖、圖表、格式、持久化、receipt 與 validator failure 都必須先局部重做 owning stage；只有三次有界恢復均耗盡且無法排除的外部 execution blocker 才可停止整輪。
