...
2. connector 直接讀取 `https://api.github.com/repos/robert820728-star/global-news-brief/branches/main?cache_bust=<nonce-a>`，取得 `commit.sha`；同輪再用另一個新 nonce 讀取 `https://api.github.com/repos/robert820728-star/global-news-brief/commits/main?cache_bust=<nonce-b>`，取得 `sha`。This is a single named `main` branch lookup, not branch enumeration；不得改用 connector 不允許的 `/git/ref/heads/main`。
