...
- A regional-supplement discovery failure does not block another covered section. When the run includes the fallback/global section, `GLOBAL_SECTION_PRIMARY_DISCOVERY_GATE` additionally requires a successful configured primary aggregator; without it, remain at source-scan even when regional candidates exist. A same-source browser snapshot is the final route fallback; cross-source web search may find same-source leads or verification evidence but cannot inject canonical candidates or satisfy route coverage.
...
12. segmented fallback 才逐 block 讀取 chunks，依既有 16-line grouped／8-line fallback、SHA-256 與 bounded retry 契約驗證；不得用 connector 搬運未驗證 helper，也不得重啟已驗證部分。
