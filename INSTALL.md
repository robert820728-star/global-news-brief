...
- ChatGPT 專案不是安裝條件；不得要求使用者建立專案，也不得宣稱排程結果可以自動歸入專案。
...
不得把日期放入固定對話名稱；日期只放在每日內容中。不得嘗試建立、指定或驗收 ChatGPT 專案。
...
- 若介面未採用指定的結果對話名稱，只能如實回報名稱控制未生效；不得把專案歸屬加入驗收項目。
- 若測試失敗，只修正失敗環節，不重新詢問已確認的偏好。

## 分享方式

接收者在自己的新對話貼上 repo 網址並執行啟動指令。每個人各自授權建立排程並保存自己的偏好，不需要建立或複製 ChatGPT 專案。
