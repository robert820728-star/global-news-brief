# 每日新聞安裝指引

本文件供 ChatGPT 或 Codex 在使用者於新對話貼上本 GitHub repo 網址後執行初始化。不得把安裝流程或後台限制寫進每日新聞讀者版。

## 安裝目標

1. 下載並啟用本 repo 的五個新聞技能；不得只把技能檔案當成一般參考文字。
2. 以最少問題建立使用者個人偏好。
3. 建立名為「每日新聞」的每日獨立排程。
4. 每次執行建立獨立結果對話，對話名稱固定為「每日新聞」。
5. 每份內容第一行顯示當天日期：`YYYY/MM/DD 每日新聞`。
6. 每次執行重新讀取本 repo 最新規則與排程保存的個人偏好。
7. 首次安裝後立即執行一次測試並驗收。

## 使用者啟動指令

使用者可在全新對話貼上 repo 網址並輸入：

> 請讀取此 GitHub repo 的 INSTALL.md。先從 `.agents/skills/` 下載並安裝完整的五個新聞技能及其必要檔案，逐一驗證後，再協助我設定個人偏好與每日新聞排程。不得只讀取 `SKILL.md` 就宣稱安裝成功；任何建立或修改技能與排程的動作都先取得我的確認。

收到指令後直接開始安裝，不要求使用者先理解 YAML、三碼代碼、排程語法、技能目錄或地圖資料格式。

## 零、下載並啟用技能

在詢問新聞偏好或建立排程前，先處理技能。必須依序執行：

1. 判斷目前環境是否支援從 GitHub repository 安裝技能。
2. 若有技能安裝器、技能建立器或等效安裝能力，先取得一次安裝授權，再從本 repo 下載並安裝下列五個完整資料夾：
   - `.agents/skills/daily-news-brief/`
   - `.agents/skills/select-news-events/`
   - `.agents/skills/verify-news-events/`
   - `.agents/skills/build-news-maps/`
   - `.agents/skills/collect-news-images/`
3. 每個技能都必須下載完整資料夾，包括 `SKILL.md`、`references/`、`scripts/`、`assets/` 與 `agents/` 中實際存在且被技能引用的內容；不得只複製單一 `SKILL.md`。
4. 逐一確認技能名稱、說明、必要參考檔與路徑可讀，並使用環境提供的技能驗證功能驗證。
5. 只有五個技能都可被技能系統辨識或明確調用時，才可回報「技能安裝完成」。

### 安裝能力不存在時

- 不得假稱已下載、已安裝或已永久保存。
- 若目前正在本 repo 的工作目錄內執行，改用 repo 級技能：從 `.agents/skills/` 載入五個技能。
- 若只是 ChatGPT 對話且只能讀取 GitHub，改用遠端讀取模式：本次安裝及每次排程執行都重新讀取五個技能與必要參考檔。
- 清楚告知使用者目前採用的是「repo 級載入」或「每次遠端讀取」，而不是永久安裝；取得使用者同意後才繼續建立排程。

### 一致性要求

- 安裝或載入的技能必須來自使用者提供的同一個 repo 與同一分支，不得自行換成同名技能或重新改寫內容。
- 已安裝技能是安裝當下的副本；每日排程仍須依 `daily-schedule-prompt.md` 重新讀取 repo 最新版核心規則，避免日後 repo 更新而技能副本停在舊版。
- 五個技能是同一套流程，缺少任何一個都視為不完整安裝，不得建立宣稱使用完整流程的排程。

## 一、驗證來源

- 確認 repo 可讀取。
- 確認下列核心檔案存在：
  - `.agents/skills/daily-news-brief/SKILL.md`
  - `.agents/skills/select-news-events/SKILL.md`
  - `.agents/skills/verify-news-events/SKILL.md`
  - `.agents/skills/build-news-maps/SKILL.md`
  - `.agents/skills/collect-news-images/SKILL.md`
  - `schemas/news-event-manifest.schema.json`
  - `scripts/validate_news_brief.py`
  - `news-brief-settings.md`
  - `news-brief-template.md`
  - `news-brief-examples.md`
  - `user-preferences.example.yaml`
  - `daily-schedule-prompt.md`
- 若核心文件缺失或互相衝突，先回報，不建立不完整排程。

## 二、只詢問三件事

### 問題一：監控板塊

> 是否要自訂新聞監控板塊？板塊可以是單一國家，也可以是區域，例如台灣、中國、日本、美國、歐盟、北美、非洲或東南亞。若不自訂，使用預設的「台灣、中國、世界」。

- 板塊數量不限。
- 台灣與中國預設獨立；只有使用者明確要求才可合併。
- 使用者只需提供自然語言名稱；成員範圍與三碼代碼由後台解析。
- 只有名稱確實有歧義時才追問。

### 問題二：主題偏好

> 是否要提高或降低某些新聞主題的權重？若沒有，使用預設設定。

- 只記錄使用者明確提出的差異。
- 未指定項目沿用 repo 預設。
- 個人偏好不得降低來源驗證、圖片驗收、地圖定位與安全規則。

### 問題三：執行時間

> 希望每天幾點產生新聞簡報？

- 時區優先使用使用者帳號、裝置或目前工作區時區。
- 只有時區無法判斷、跨時區使用或使用者主動指定時才追問。
- 不要求使用者填寫排程語法。

## 三、自動判斷

### 語言

1. 優先使用使用者已明確設定的輸出語言。
2. 沒有設定時，使用安裝對話的主要語言。
3. 只有無法可靠判斷或使用者要求變更時才詢問。
4. 語言套用於板塊名稱、欄位、標題、內文、地圖圖說與圖片圖說；來源原名及網址可保留。

### 板塊代碼

- 每個板塊配置唯一的三個大寫英文字母。
- 國家優先使用 ISO 3166-1 alpha-3；區域或組織使用清楚、穩定且不衝突的三碼。
- 事件編號格式固定為 `XXX-01`、`XXX-02`。

### 篇幅與門檻

- 未指定時沿用 `user-preferences.example.yaml` 的預設值。
- 板塊增加時不得為了填滿每區而加入低價值新聞。
- 同一事件只放在一個主要板塊，跨國系統性事件才優先放全球板塊。

## 四、建立每日獨立排程

取得使用者排程授權後，依 [daily-schedule-prompt.md](daily-schedule-prompt.md) 建立排程：

- 排程名稱：每日新聞。
- 頻率：每天一次。
- 執行時間與時區：依個人偏好。
- 模式：每次建立獨立結果對話，不返回安裝對話，也不堆疊於前一天對話。
- 結果對話名稱：每日新聞。
- 內容日期：每份讀者版第一行固定為執行地日期的 `YYYY/MM/DD 每日新聞`。
- 新聞時間窗：每次實際執行時間往前精確 24 小時。
- 規則來源：每次重新讀取 GitHub 最新版核心文件。
- 個人偏好：保存在使用者自己的排程設定，不回寫公共 repo。
- 執行流程：固定由 `daily-news-brief` 依序調用海選、複查、地圖、圖片技能，再套用模板；不得把整個流程改寫成單一臨時提示詞。

## 五、首次測試

- 排程建立後立即手動執行一次，不必等待隔日。
- 檢查：
  - 五個技能是否真的被技能系統辨識；若採遠端讀取模式，是否完整讀取必要參考檔。
  - 是否建立獨立結果對話，而非回到安裝對話。
  - 排程與可控制的結果對話名稱是否為「每日新聞」。
  - 內容第一行是否為當天的 `YYYY/MM/DD 每日新聞`。
  - 板塊、三碼事件編號、語言、時區、24 小時時間窗、來源、地圖、圖片與三個頂層區塊是否正確。
  - 所有事件是否都完成多來源搜尋；只有一個可靠來源時是否照常收錄、保留原等級並顯示固定來源限制文字。
  - 地圖與圖片是否同時保留，且沒有因後段處理而覆蓋來源、標題、等級或其他附件。
- 若環境可執行 Python，另執行 `python3 -m unittest discover -s tests -v` 確認驗證器正常。
- 若測試失敗，只修正失敗環節，不重新詢問已確認的偏好。

## 分享方式

接收者在自己的新對話貼上 repo 網址並執行本文件的啟動指令。每個人各自授權下載技能、建立排程並保存自己的偏好。若對方環境不支援技能安裝，必須明確採用 repo 級載入或每次遠端讀取，不得把「看過技能檔案」說成「已安裝技能」。
