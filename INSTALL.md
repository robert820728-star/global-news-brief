...
`SCHEDULED_NATIVE_IMAGE_SEARCH_FIRST_GATE`：真實 Scheduled Task 驗收已證明原生圖片搜尋可直接產生對話可見 image ref，而單一直接 JPEG URL 可能只得到 fetch/cache miss。因此 Scheduled Task 對每則入選事件的第一個可見交付動作必須是實際呼叫原生圖片搜尋，不是一般 web search。先從原引用文章取得精確標題、caption、攝影者、日期與地點，以這些身分資料搜尋原引用來源；沒有合格 ref 時不得離開事件或宣告失敗，必須依序改用官方／當事組織、原始通訊社、事件所在地的當地可靠媒體與當地語言、其他可靠媒體，且每換來源都重新用「精確事件＋日期＋地點＋新來源」呼叫原生圖片搜尋。逐張核對同一 semantic event 與日期；取得合格 image ref 後立即交付原生圖片卡。Scheduled Task 在上述跨來源原生搜尋尚未完成前，不得因原文章圖片、CDN、直接 JPEG 或下載路徑未形成 ref 而停下，也不得先卡進反覆直接媒體下載；完成跨來源原生搜尋仍無合格 ref 時，才進入 `DIRECT_ARTICLE_MEDIA_DELIVERY_ROUTE` 與宿主實際可用的頁面／圖片區域截圖。
...
`FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：Scheduled Task 固定先完成跨來源原生圖片搜尋：原引用來源 → 官方／當事組織 → 原始通訊社 → 當地可靠媒體與當地語言 → 其他可靠媒體；每換一層都以精確事件、日期、地點及新來源重新搜尋，任一合格 `image_ref` 立即放入最終原生 image group／圖片卡。全部跨來源原生搜尋仍無合格 ref 時，才走原文章或各替代來源的 `img`／`srcset`／`og:image`／圖集直接媒體，再用宿主實際存在的頁面／圖片區域截圖；URL 或下載失敗不得把流程鎖死。full-runtime 使用既有 `scripts/materialize_news_images.py` 下載實際 bytes，下載失敗立即改以該頁圖片區域的本機 `screenshot_path`，完成 MIME、解碼、尺寸、SHA-256 與內容核對後交付本機實體附件。成功即停止該事件重試；單一路徑失敗不得升格成整體 blocker。
