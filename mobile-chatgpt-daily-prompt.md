...
7. 圖片內容沿用原先為該則新聞選定的圖片，不得為了縮小檔案改換另一張圖。`IMAGE_DEFAULT_ONE_ASSET`：每則預設一張內嵌圖片；`IMAGE_SECOND_ASSET_REQUIRES_INCREMENTAL_INFORMATION`：只有第二張能補充第一張未呈現的範圍、數字、現場或時間變化時才追加，並記錄新增資訊理由，每則最多兩張：
   ...
   - `LOCAL_ATTACHMENT_FIRST_WHEN_RUNTIME_AVAILABLE_GATE`：若本輪 capability probe 顯示可寫檔案系統與可執行 runtime，這不再是 mobile-only 圖片路徑；逐則解析原文章／官方／可靠轉載媒體，下載或截圖成實體圖片檔，驗證後以本機附件交付。原生圖片搜尋只可作為候選定位，不得取代本機附件。
   - `MOBILE_NATIVE_IMAGE_SEARCH_FALLBACK_GATE`：只有確實沒有可寫檔案系統或可執行 runtime 時才用原生圖片卡。卡片必須形成非文字媒體內容塊；若結果只出現在一般文字中的 `image_group`／`image_ref`，立即判失敗並改用同一來源頁圖片區域截圖，再依官方／當事組織、原始通訊社、當地可靠媒體與其他可靠媒體順序繼續。不得重複輸出 ref 或等待。
   ...
   - `FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：先依實際能力選較強路徑。有可寫檔案系統與 runtime 時，依原引用來源 → 官方／當事組織 → 原始通訊社 → 當地可靠媒體與當地語言 → 其他可靠媒體逐層下載或截圖成實體檔並交付本機附件。確實無本機能力時才用原生圖片搜尋定位並嘗試非文字圖片卡；文字化 ref 立即失敗，改用頁面圖片區域截圖再進下一來源。沒有截圖工具時不得把「應該截圖」記為已嘗試；任一路徑成功後立即交付並停止該事件重試。
