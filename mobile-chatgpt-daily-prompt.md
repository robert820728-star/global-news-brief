...
5. 維護此排程對話內的十四天滾動海選清單。新增本輪候選、合併同事件更新，並移除超過十四天的項目；每筆仍須保留六項大評分、總分、等級、決定與理由。
   ...
   - `MOBILE_NATIVE_AUDIT_ROLLING_MERGE`：`latest-candidate-audit.json` 已存在時，mobile-native 直接以該檔為滾動基底。canonical durable audit 只保留符合目前 schema 與 `public_value_v2` 的 run；不相容物件不合併，由 Git history 保存追溯資訊。移除超過十四天項目、按 `dedup_key`／`continuity_key` 合併本輪增量，並只重評本輪新增或有實質更新的候選。需要更新 durable audit 時，允許使用 GitHub contents API 整檔 replacement 寫回語意等同的合併結果，不要求本機程式；C 級以上事件仍須依本輪證據政策獨立驗證。這項 mobile-native 合併不得冒充 script validation，但也不得因此阻止本日讀者版。
