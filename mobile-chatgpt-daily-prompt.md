...
- `EVENT_REGION_AND_TIME_IDENTITY_GATE`: 語意聚類完成後、六項評分前，從事件內容獨立建立 `country_codes`、`primary_country_code`、`location_evidence`、`event_occurred_at`、`material_update_at`、`material_update_type` 與 `material_update_evidence`。來源分桶及媒體國別不得作為事件地區；事件板塊只能由事件本身的主要國家／區域推導。文章今天發布不代表事件今天發生；舊事件若只是重新整理、回顧、週年、換標題或重刊，逐列標為 `non_news`，不得建立新事件。只有 `material_update_at` 位於精確 24 小時窗內且有可驗證的官方確認、傷亡／影響修正、政策／法律變化、狀態轉折或實質升降級，才可沿用原事件身分成為延續更新。缺漏或矛盾一律 `unresolved`，不得評分或刊出。
