...
6. 完成評分後，將本輪精確 24 小時增量寫入 run-scoped candidate audit：`run-logs/logs/runs/<run_id>/candidate-audit.json`。它與十四天 durable history 是兩個 artifact；前者保存本輪所有已取得語意事件、六項分數、總分、等級、決定、理由、來源、`selected_event_id`、可取得的 `article_dispositions` 與 counts，不能因 durable merge 延後而刪除。Verification 完成後保存 `run-logs/logs/runs/<run_id>/verification.json`；地圖需求判定完成後保存 `run-logs/logs/runs/<run_id>/map-decisions.json`。完整讀者版產生後，將同一 bytes 寫入本輪 `reader.md` 與 `logs/latest-reader.md`；同時持久化 mobile-native 確實能產生的 image evidence、map decisions、run-scoped audit、reader 與 ledger reference，最後才原子更新 `logs/current.json`。不得建立或冒充通過只適用 full-runtime 本機附件契約的 `news-event-manifest.json`。`candidate_audit_artifact` 必須指向本輪 run-scoped candidate audit；進入 `visuals-completed` 前 `verification_artifact` 必須綁定本輪 `verification.json`；進入 `reader-rendered` 前 `map_decisions_artifact` 必須綁定本輪 `map-decisions.json`；宣告媒體能力降級時 `image_evidence_artifact` 必須指向本輪 `image-evidence.json`；`durable_audit_artifact` 才指向 `logs/latest-candidate-audit.json`。這兩個新增 binding 只證明已完成成果可由同一 run 恢復，不宣稱其內容另經新的 machine validator 驗證。
