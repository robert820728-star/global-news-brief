...
- A discovery source failure must not block the whole brief. Record the degraded source and continue when at least one configured discovery route yields verifiable current candidates. Cross-source web search may support same-source recovery or later verification but cannot inject canonical candidates or satisfy route coverage. Stop only when no configured-route candidate can be verified at all.
...
1. 排程外層為取得最新版規則而進行的 `external latest-main resolution` 與 pinned prompt read 不計入本段順序；它們不得讀取新聞或舊成果。載入本 prompt 後，`first runtime GitHub action` 才是讀取 `run-logs/logs/current.json`。同一 `scheduled_for` 是唯一 occurrence key：只要該 occurrence 的 current record 已存在，不論停在 `awaiting_executor`、`running`、`reader-rendered` 或其他非 terminal 階段，都沿用其中的 `run_id` 與最後階段，從 first incomplete stage 接續；不得建立 replacement run、不得旋轉 current、不得重跑已完成新聞階段。05:58 守望工作已建立當天 `status=awaiting_executor` 時，立即把目前階段更新成 `executor-started`、`status=running`。
...
5. 只有會使當日本輪事件無法核實、無法完成 C 級以上 mapping 或無法產生讀者文字的錯誤，才將 `status=failed` 並寫入 `last_error`。可重算 count 差額、十四天歷史維護延後、單一 discovery route 降級及原生媒體能力限制都依各自的非阻塞規則處理，不得冒充 fatal error。突然中斷時，GitHub 保留最後一次成功更新；同一 `scheduled_for` 必須恢復該紀錄，只有下一個真實 occurrence 才能把仍非 terminal 的前輪標成 `interrupted_by_next_run`。
6. 完成評分後，將本輪精確 24 小時增量寫入 run-scoped candidate audit：`run-logs/logs/runs/<run_id>/candidate-audit.json`。它與十四天 durable history 是兩個 artifact；前者保存本輪所有已取得語意事件、六項分數、總分、等級、決定、理由、來源、`selected_event_id`、可取得的 `article_dispositions` 與 counts，不能因 durable merge 延後而刪除。完整讀者版產生後，將同一 bytes 寫入本輪 `reader.md` 與 `logs/latest-reader.md`；同時持久化該執行模式能產生的 image evidence、map decisions、checkpoint、manifest 與 receipts，最後才原子更新 `logs/current.json`。`candidate_audit_artifact` 必須指向本輪 run-scoped candidate audit；宣告媒體能力降級時 `image_evidence_artifact` 必須指向本輪 `image-evidence.json`；`durable_audit_artifact` 才指向 `logs/latest-candidate-audit.json`。
   ...
   - `CANONICAL_THREE_PART_READER_LAYOUT_GATE`：`## 逐條詳報` 依 manifest 順序輸出每則事件，標題為「事件編號. 事件名稱 - 等級」，固定欄位順序為 `時間／來源／地圖／資料圖表／圖片／事件細節／各方說法／分析`；時間、來源、事件細節、分析必填，其餘依 manifest 條件式顯示。沒有實際可見附件時省略對應視覺欄位，不得顯示圖片說明、caption 或占位文字。兩則事件之間必須有 `---`；`## 後續觀察` 逐字使用 manifest 的具體 follow-up。只有可執行 runtime 時才執行 `scripts/validate_news_brief.py brief --manifest <manifest> --input <reader>`。`MOBILE_READER_STRUCTURE_EQUIVALENT`：mobile-native 直接逐項核對三個二級標題及順序、overview 與 manifest 事件 ID 守恆、逐事件必填欄位、分隔線數量、follow-up 逐字一致及零附件零圖說；格式錯誤時只重做 reader render，不重跑新聞階段，也不得宣稱執行過 script validator。
...
7. canonical reader、當輪新聞證據、run-scoped candidate audit 與該執行模式可執行的驗證通過後，輸出對話前最後一次持久更新可為 `delivery-handoff`、`status=completed`、`delivery_status=handoff_started`。十四天 durable history 為 `updated`、`preserved_merge_deferred` 或 `current_run_only` 都不阻擋本輪完成。`MOBILE_NATIVE_IMAGE_EVIDENCE_ROUTE`：full-runtime 實際執行下載、失敗後截圖、物化驗收及附件交付；mobile-native 實際執行來源檢查、原生圖片搜尋／圖片卡嘗試並保存宿主結構化結果。若該模式可用的交付路徑已窮盡仍無法交付媒體，改記 `delivery_profile=reader-canonical-capability-degraded`、`native_media_status=unavailable` 與 `capability_limitations=[NATIVE_MEDIA_UNAVAILABLE]`；`last_error` 必須為空。不得捏造本地下載、截圖、檔案、附件或像素驗收。此狀態只有在 `image_evidence_artifact` 指向本輪 `logs/runs/<run_id>/image-evidence.json` 時才可 completed；缺少該 artifact 必須停在 visuals／handoff，不得宣稱已嘗試。即使已 handoff，沒有外部明確回執時仍不得宣稱 `client_confirmed` 或手機畫面已收到。
...
7. 圖片內容沿用原先為該則新聞選定的圖片，不得為了縮小檔案改換另一張圖。`IMAGE_DEFAULT_ONE_ASSET`：每則預設一張內嵌圖片；`IMAGE_SECOND_ASSET_REQUIRES_INCREMENTAL_INFORMATION`：只有第二張能補充第一張未呈現的範圍、數字、現場或時間變化時才追加，並記錄新增資訊理由，每則最多兩張：
   ...
   - `NATIVE_MEDIA_CAPABILITY_FALLBACK`：不得根據工具名稱或預檢結果直接宣告 `NATIVE_MEDIA_UNAVAILABLE`。能執行 runtime 時先用 `scripts/materialize_news_images.py --input <image-candidates.json> --output-dir <materialized-image-dir> --manifest <materialized-images.json>`；原圖下載失敗才截圖，取得合格本機 JPEG／WebP 後實際嘗試附件或原生媒體交付。沒有 runtime 的 mobile-native 則逐則執行來源檢查、`NATIVE_IMAGE_SEARCH_CARD_ROUTE` 與宿主可提供的結構化交付檢查；不得捏造本地路徑、下載、截圖、物化、附件或像素驗收。只有該模式的實際交付嘗試仍失敗，才記錄 capability-degraded 狀態。完全找不到合格圖片必須記為 source exhaustion，不得冒充 `NATIVE_MEDIA_UNAVAILABLE`。將 verified image evidence、取得／交付嘗試與內部 `reader_omission_note` 寫入本輪 `image-evidence.json`，並以 `image_evidence_artifact` 保存其 Git blob SHA；該 SHA 只證明 evidence 已保存，不代表內容或像素已經 machine-verified。缺少 artifact 不得 completed；不得建立新 run 或重跑新聞流程。不得因宿主缺少原生媒體能力阻擋正式文字交付，但也不得把未執行的媒體步驟寫成已完成。
