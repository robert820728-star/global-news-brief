...
7. 圖片內容沿用原先為該則新聞選定的圖片，不得為了縮小檔案改換另一張圖。`IMAGE_DEFAULT_ONE_ASSET`：每則預設一張內嵌圖片；`IMAGE_SECOND_ASSET_REQUIRES_INCREMENTAL_INFORMATION`：只有第二張能補充第一張未呈現的範圍、數字、現場或時間變化時才追加，並記錄新增資訊理由，每則最多兩張：
   ...
   - `SCHEDULED_NATIVE_IMAGE_SEARCH_FIRST_GATE`：每則入選事件先實際呼叫原生圖片搜尋，不得以一般 web search、開文章頁或解析圖片 URL 代替。先從原引用文章取得精確標題、caption、攝影者、日期與地點並搜尋原引用來源；沒有合格 ref 時，依官方／當事組織、原始通訊社、當地可靠媒體與當地語言、其他可靠媒體逐層更換來源，每次以精確事件／日期／地點及新來源重新搜尋。取得合格 ref 後立即交付；跨來源原生搜尋完成前不得卡進直接 URL／下載或宣告失敗。全部仍無 ref 時才走直接文章媒體與宿主可用截圖。
   ...
   - `FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：固定先完成原引用來源 → 官方／當事組織 → 原始通訊社 → 當地可靠媒體與當地語言 → 其他可靠媒體的跨來源原生圖片搜尋，並把合格 `image_ref` 放入最終 image group／圖片卡；全部仍無 ref 時才試各來源直接媒體與宿主實際存在的頁面圖片區域截圖。沒有截圖工具時不得把「應該截圖」記為已嘗試；任一路徑成功後立即交付並停止該事件重試。
