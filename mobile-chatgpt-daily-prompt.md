...
- A regional-supplement discovery failure does not block another covered section. When the run includes the fallback/global section, `GLOBAL_SECTION_PRIMARY_DISCOVERY_GATE` prefers a successful configured primary aggregator. After every configured primary route is exhausted, verified cross-source global results may enter the audit only as the reserved `web_fallback` row. The row must preserve replayable search evidence, the exact window and every admitted URL; it remains `coverage_complete=false` / `degraded_partial`, cannot satisfy configured-route completeness, and cannot impersonate GDELT. If neither route works, remain at source-scan even when regional candidates exist.
- `MOBILE_DISCOVERY_TRANSPORT_ADAPTERS`: execute only transports the Scheduled Task host actually exposes. For GDELT, use the official export archive only when binary fetch and ZIP decode are available; otherwise use the single allowed DOC request, then the existing `web_fallback`, preserving `degraded_partial`. For CNA, use the canonical POST when available; otherwise traverse the same-site public list or search pages and retain every visited page URL, pagination/boundary evidence, then open each admitted article. For China News Service, fetch the configured day-0 and day-minus-1 scroll pages; if direct fetch fails, use a same-site alternate or browser snapshot without claiming complete coverage unless both date variants and the window boundary are proven. An adapter failure remains explicit source coverage evidence and never becomes a fabricated zero-news result.
- `ARTICLE_BODY_TIMESTAMP_AUTHORITY_GATE`: CNA and every browser/search recovery candidate is admitted to the 24-hour window only from the article body（文章本體）所載、可歸屬的 publication/update time. List/search snippets are leads, not time authority; a stale or mismatched article body is rejected from the window.
- `SEARCH_OPEN_FRESHNESS_CONSISTENCY_GATE`: bind each search result and subsequently opened page by canonical URL, title/content identity and publication time. If the search snapshot and opened content disagree, mark `stale-open`; do not use that open as source exhaustion, a window boundary, or evidence that the newer result was inspected.
...
- Browser/web search normally locates a same-source URL for a configured discovery route or original evidence for later verification. The only discovery exception is the bounded global recovery above: after every primary-aggregator route is exhausted, individually verified results may enter through `web_fallback`; they must use the same scoring and verification path and can never satisfy configured-route completeness.
...
5. 只有會使當日本輪事件無法核實、無法完成 C 級以上 mapping 或無法產生讀者文字的錯誤，才將 `status=failed` 並寫入 `last_error`。可重算 count 差額、十四天歷史維護延後及區域補充 route 降級依各自規則處理。`GLOBAL_SECTION_PRIMARY_DISCOVERY_GATE`：本輪包含 fallback／全球板塊且 GDELT 的 archive、一次 DOC API 與有效 cache 全部不可用時，立即執行受控全球 web search；逐筆保存搜尋結果、發稿時間、原始網址與快照，並以 `web_fallback` source coverage row 進入相同去重、語意事件、評分與驗證流程。它不得宣稱完整 coverage 或冒充 GDELT。只有 primary 與受控備援都未取得可核實候選時，才讓同一 run 保持 `status=running`、`current_stage=source-scan`；CNA／中新社本身不能把世界 coverage 缺失表示成零事件。突然中斷時，GitHub 保留最後一次成功更新；同一 `scheduled_for` 必須恢復該紀錄，只有下一個真實 occurrence 才能把仍非 terminal 的前輪標成 `interrupted_by_next_run`。
...
7. canonical reader、當輪新聞證據、run-scoped candidate audit 與該執行模式可執行的驗證通過後，且不存在未解決的媒體交付失敗時，輸出對話前最後一次持久更新才可為 `delivery-handoff`、`status=completed`、`delivery_status=handoff_started`。十四天 durable history 為 `updated`、`preserved_merge_deferred` 或 `current_run_only` 都不阻擋本輪完成。`MOBILE_NATIVE_IMAGE_EVIDENCE_ROUTE`：full-runtime 可下載或截圖後物化附件；Scheduled Task 宿主實際執行來源檢查、文章直接媒體 URL、原生圖片搜尋／圖片卡、頁面圖片區域直接截圖及後續來源嘗試並保存宿主結構化結果。`QUALIFIED_IMAGE_DELIVERY_INDEPENDENT_OF_CLAIM_CRITICAL`：若已確認存在合格來源圖片，就必須在同一 occurrence 以原生圖片卡或直接截圖完成可見交付；不得在 discovery 後宣告 `NATIVE_MEDIA_UNAVAILABLE`、不得等待未實際存在的未來 full-runtime worker，也不得標記 canonical completed。完全沒有合格來源圖片才是 source exhaustion。即使已 handoff，沒有外部明確回執時仍不得宣稱 `client_confirmed` 或使用者畫面已收到。
   ...
   - `MULTIMODAL_READER_ORDERED_BLOCK_CONTRACT`：最終訊息由 ordered text／media blocks 組成；串接所有 text blocks（忽略 media blocks）必須逐 byte 等於 `logs/latest-reader.md`。每個原生媒體 block 插在其事件文字中的 `media anchor`；caption 是 canonical Reader text 並緊接 anchor，媒體 metadata 不得另寫或改寫 caption。媒體插入不改變 saved Reader bytes。
...
- `SCHEDULE_AND_TIMEZONE_OCCURRENCE_AUTHORITY`：排程時間與 timezone 只讀 Scheduled Task 的 task／occurrence；本檔不得自行指定或覆寫。使用者未指定時的安裝預設由 `INSTALL.md` 處理。
...
7. 圖片內容沿用原先為該則新聞選定的圖片，不得為了縮小檔案改換另一張圖。`IMAGE_DEFAULT_ONE_ASSET`：每則預設一張內嵌圖片；`IMAGE_SECOND_ASSET_REQUIRES_INCREMENTAL_INFORMATION`：只有第二張能補充第一張未呈現的範圍、數字、現場或時間變化時才追加，並記錄新增資訊理由，每則最多兩張：
   ...
   - `VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE`：只有本輪已證明 `source_media_byte_fetch`（或實際可用的 `webpage_region_screenshot`）、解碼／尺寸／雜湊驗證與 `local_attachment_media_handoff` 全部成立時，才優先交付本機附件。Python／可寫檔案系統本身不構成完整圖片 pipeline；鏈上任一能力缺失時改走另一條已驗證媒體路徑。
   - `MOBILE_NATIVE_IMAGE_SEARCH_FALLBACK_GATE`：當完整 `VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE` 不成立時，才使用本輪已實測可用的原生圖片卡或其他媒體輸出。卡片必須形成非文字媒體內容塊；若結果只出現在一般文字中的 `image_group`／`image_ref`，立即判失敗。只有 `webpage_region_screenshot` 已獨立實測成功時才改用同一來源頁圖片區域截圖；否則依官方／當事組織、原始通訊社、當地可靠媒體與其他可靠媒體順序繼續下一個已驗證路徑。不得重複輸出 ref 或等待。
   - `NATIVE_IMAGE_QUERY_RESULT_IS_NOT_CAPABILITY_GATE`：原生圖片搜尋工具已可呼叫、但某次查詢沒有回傳合格 `image_ref`，不代表宿主缺少圖片交付能力。不得在 discovery 前或後續 stage 把單次零結果、錯圖、舊圖或來源未索引改判成 `HOST_VISIBLE_MEDIA_TRANSPORT_UNAVAILABLE`；只有工具本身不存在、呼叫被拒絕或無法建立任何原生圖片結果物件時才是 capability failure。其餘情況必須繼續同事件替代來源與查詢。
   - `WIRE_PROVIDER_SUBSTITUTION_GATE`：通訊社文章有當期圖片但查詢沒有合格 ref 時，不得反覆查詢同一通訊社、caption 或 CDN。第一來源為 Reuters 時，下一個通訊社查詢優先改用 AP 同事件報導，再查官方／當事組織、當地可靠媒體與當地語言事件查詢，最後才查其他可靠媒體；每次替換都以精確事件／地點／日期及新來源名稱重新做原生圖片搜尋，合格 ref 出現即交付。
   - `CURRENT_EVENT_CONTEXT_PHOTO_GATE`：可靠媒體在同一篇當期事件報導中使用、且可核對為同日同地的現場脈絡照片可作為合格當期新聞圖片，不要求直接拍到核心行動發生的一瞬間。以精確文章標題、來源名稱及可取得的 caption／攝影者／地點重新做原生圖片搜尋；圖說必須忠實描述畫面，不得描述成直接拍攝核心行動。其他年份、其他地點、歷史檔案或泛用示意圖仍不得入選。
   - `RELATIVE_MEDIA_URL_RESOLUTION_GATE`：文章媒體欄位若提供協定相對、根目錄相對或路徑相對位址，必須以重新導向後的文章 URL 為基準解析；有效 `base href` 優先套用，再產生絕對 HTTP(S) URL。不得把相對路徑本身當成下載失敗或來源無圖。
   - `SAME_OCCURRENCE_NATIVE_IMAGE_REF_GATE`：原生 `image_ref` 必須由目前 occurrence 的實際圖片工具結果建立，並在同一最終訊息真正渲染成 image group／圖片卡。前一個 task、其他對話或其他 occurrence 的 ref 文字不可重用；只列 ref id、聲稱已建立、或詢問「如果你看得到」都算未交付。
   - `FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：先依 `INDEPENDENT_VISIBLE_MEDIA_CAPABILITY_PROBE` 的結果選端到端路徑。完整 local pipeline 成立才走 bytes／真正截圖→驗證→本機附件；否則用已驗證的原生圖片卡或其他媒體輸出。`page_open` 不得推導 `webpage_region_screenshot`；缺少截圖工具就跳過該路徑，不得把「應該截圖」記為已嘗試。任一路徑成功後立即交付並停止該事件重試。
   - `DIRECT_ARTICLE_MEDIA_DELIVERY_ROUTE`：原生圖片搜尋／圖片卡不是唯一取得路徑。逐則檢查原引用文章的 `img`、`srcset`、`og:image` 或等價欄位；若找到與當期事件相符的直接 JPEG／WebP URL，實際以宿主可用媒體路徑開啟／取得並嘗試可見交付。搜尋卡沒有 image ref 不等於圖片不可取得；外部 URL、Markdown 圖片字串或純文字連結本身不算可見交付。
...
10. Scheduled Task 宿主無法產生地圖或圖表等本機生成資產時，可依既有非關鍵視覺規則記錄 omission；但已確認存在的合格來源圖片不得省略，必須在同一 occurrence 繼續原文、官方、通訊社、可靠轉載及直接截圖／原生圖片卡路徑，成功交付後才進 Reader。不得等待未實際存在的未來 full-runtime worker，也不得重跑新聞或改變既有 reader 事實。

`VISUAL_DELIVERY_ONLY_RECOVERY`：同一 occurrence 若新聞、評分與驗證均已完成而仍缺圖片，只讀既有 candidate audit、verification、map decisions、image evidence 與來源頁，只重做未交付事件的原生圖片卡或頁面圖片區域直接截圖。禁止 discovery、scoring、verification、new run 或 event-ID 變更。
