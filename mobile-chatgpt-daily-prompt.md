...
6. 完成評分後，將本輪精確 24 小時增量寫入 run-scoped candidate audit：`run-logs/logs/runs/<run_id>/candidate-audit.json`。它與十四天 durable history 是兩個 artifact；前者保存本輪所有已取得語意事件、六項分數、總分、等級、決定、理由、來源、`selected_event_id`、可取得的 `article_dispositions` 與 counts，不能因 durable merge 延後而刪除。完整讀者版產生後，將同一 bytes 寫入本輪 `reader.md` 與 `logs/latest-reader.md`；同時持久化該執行模式能產生的 image evidence、map decisions、checkpoint、manifest 與 receipts，最後才原子更新 `logs/current.json`。`candidate_audit_artifact` 必須指向本輪 run-scoped candidate audit；宣告媒體能力降級時 `image_evidence_artifact` 必須指向本輪 `image-evidence.json`；`durable_audit_artifact` 才指向 `logs/latest-candidate-audit.json`。
   ...
   - `CANONICAL_SECTIONED_READER_LAYOUT_GATE`：今日總覽後必須依相同板塊順序放逐區新聞；每則固定為「標題｜評級 → 實際可見附件（若有）→ 新聞摘要 → 評為X級的評論與段末來源」。沒有附件時不得顯示圖片說明、caption 或占位文字。不得改成欄位式逐條詳報，不得顯示事件編號，也不得產生 `時間／來源／事件細節／分析` 欄位組。送出前必須執行 `scripts/validate_news_brief.py brief --reader-layout canonical-sectioned`；格式錯誤時只重做 reader render，不重跑新聞階段。
