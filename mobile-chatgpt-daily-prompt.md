...
7. 圖片內容沿用原先為該則新聞選定的圖片，不得為了縮小檔案改換另一張圖。`IMAGE_DEFAULT_ONE_ASSET`：每則預設一張內嵌圖片；`IMAGE_SECOND_ASSET_REQUIRES_INCREMENTAL_INFORMATION`：只有第二張能補充第一張未呈現的範圍、數字、現場或時間變化時才追加，並記錄新增資訊理由，每則最多兩張：
   ...
   - `MOBILE_B_OR_HIGHER_VISIBLE_IMAGE_GATE`：C 級新聞可使用圖片說明，但 B 以上事件至少要有一張在本對話實際可見、可確認與該事件相關的來源圖片或圖片卡。若讀者版含 B 以上事件，則不得整份零張可見圖片。先檢查已引用來源頁的內文圖片、`og:image`／`srcset` 與官方圖資；仍無結果時，再檢查一個已加入來源欄的可信同事件來源。找到可用圖片卻無法顯示時，不得把 B 以上事件改成無圖完成；只重做圖片階段，不重跑 discovery、評分、驗證或 reader 文字，也不得用無關示意圖、人物舊照或來源標誌湊數。
