...
7. 圖片內容沿用原先為該則新聞選定的圖片，不得為了縮小檔案改換另一張圖。`IMAGE_DEFAULT_ONE_ASSET`：每則預設一張內嵌圖片；`IMAGE_SECOND_ASSET_REQUIRES_INCREMENTAL_INFORMATION`：只有第二張能補充第一張未呈現的範圍、數字、現場或時間變化時才追加，並記錄新增資訊理由，每則最多兩張：
   ...
   - `SCHEDULED_NATIVE_IMAGE_SEARCH_FIRST_GATE`：每則入選事件先實際呼叫原生圖片搜尋，不得以一般 web search、開文章頁或解析圖片 URL 代替。固定查詢順序為：精確標題／caption＋日期＋地點；事件＋日期＋官方／當事組織；事件＋日期＋適用的路徑圖／衛星圖／災情圖／現場照片；事件＋日期＋通訊社／可靠媒體。逐張核對同一 semantic event 與日期；取得合格 image ref 後立即交付原生圖片卡，不得繼續撞已失敗的直接圖片網址。沒有合格 image ref 才進入直接文章媒體、宿主實際可用的頁面截圖與後續來源；每換一層來源都用同一事件重新執行原生圖片搜尋。
   ...
   - `FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：固定依序執行原生圖片搜尋並把合格 `image_ref` 放入最終 image group／圖片卡 → 原文章直接媒體取得實際內容或 native ref → 宿主實際存在時截取頁面圖片區域並直接交付該截圖 → 官方／當事組織 → 原始通訊社 → 其他可靠媒體；每換一層都先重新做同事件原生圖片搜尋，再試直接媒體與截圖。沒有截圖工具時立即換來源，不得把「應該截圖」記為已嘗試；任一路徑成功後立即交付並停止該事件重試。
