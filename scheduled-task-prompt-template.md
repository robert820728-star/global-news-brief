...
ChatGPT Scheduled Task 對每一則入選事件必須實際呼叫原生圖片搜尋，並把它放在第一個可見圖片交付動作；不得只做一般 web search、只開文章頁或只解析圖片 URL。先從原引用文章取得精確標題、caption、攝影者、日期與地點，以這些身分資料搜尋原引用來源；沒有合格 ref 時不得離開事件或宣告失敗，必須依序改用官方／當事組織、原始通訊社、事件所在地的當地可靠媒體與當地語言、其他可靠媒體，且每換來源都重新用「精確事件＋日期＋地點＋新來源」呼叫原生圖片搜尋。逐張核對 image ref 的事件、日期、人物／地點與來源；其他年份、其他事件、只有關鍵字相似的舊圖全部淘汰。取得合格 image ref 後立即交付原生圖片卡，不得繼續撞已失敗的直接圖片網址。上述跨來源原生搜尋尚未完成前，不得因原文章圖片、CDN、直接 JPEG 或下載路徑未形成 ref 而停下，也不得先卡進反覆直接媒體下載；全部跨來源原生搜尋仍無合格 ref 時，才繼續文章直接媒體與頁面截圖。
