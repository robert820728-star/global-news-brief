...
- `LOCAL_ATTACHMENT_FIRST_WHEN_RUNTIME_AVAILABLE_GATE`：宿主有可寫檔案系統與可執行 runtime 時，逐則先解析原文章／官方／可靠轉載媒體，下載或截圖成實體圖片檔，通過既有驗證後以本機附件交付；原生圖片搜尋只可作為候選定位，不能取代本機附件。
- `MOBILE_NATIVE_IMAGE_SEARCH_FALLBACK_GATE`：只有無本機能力時才使用原生圖片卡；若卡片被序列化為一般文字中的 `image_group`／`image_ref`，立即判該路徑失敗並改用頁面圖片區域截圖，再依官方／當事組織、原始通訊社、當地可靠媒體與其他可靠媒體順序重試。禁止等待或重複輸出 ref。
...
- `FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：有可寫檔案系統與 runtime 時，依原引用來源 → 官方／當事組織 → 原始通訊社 → 當地可靠媒體與當地語言 → 其他可靠媒體逐層下載或截圖成實體檔，以既有 materializer 驗證後交付本機附件。確實無本機能力時才用原生圖片搜尋定位並嘗試非文字圖片卡；文字化 ref 立即失敗，改用頁面圖片區域截圖再進下一來源。單一路徑失敗不得停止。
