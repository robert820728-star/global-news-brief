...
- `逐條詳報` 每則新聞的公開欄位之間已空一行；沒有把時間、來源、事件細節、各方說法、分析擠成連續段落。
...
逐條詳報排版必須手機閱讀優先：同一則新聞內，`標題`、`時間`、`來源`、`事件細節`、`各方說法`、`分析` 之間都必須空一行。也就是每個欄位各自成段，不得把 `時間`、`來源`、`事件細節`、`各方說法`、`分析` 連續貼在一起。若省略 `各方說法`，`事件細節` 與 `分析` 之間仍須空一行。`今日總覽` 表格仍維持緊湊，不要在表格列之間插入空行。
...
- `逐條詳報` 欄位之間必須空一行，讓時間、來源、事件細節、各方說法與分析在手機上容易分段閱讀。
- `各方說法` 無用時應省略。
