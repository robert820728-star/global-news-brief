...
- Regional supplements 全數進模型；GDELT 每列完整保存在 relevance gate，只有符合非相對複合訊號的 `content_hydration` 列進入本輪模型輸入。`lightweight_semantic_review` 列保留為未完成語意審查的 coverage 證據，不得偽裝成 `non_news` 或已評分事件。禁止固定 top-N；強跨來源／文章熱度本身即可進入 `content_hydration`，不得要求先命中偏政治或災害的詞表，因此科學、科技、資安、醫學與文化事件使用相同准入標準。
