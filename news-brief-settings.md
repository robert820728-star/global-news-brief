...
保存每日新聞簡報的編輯偏好、板塊、分級與收納標準。執行流程、來源複查、地圖、圖片、自主恢復及欄位所有權由 repo 內七個技能負責，避免單一提示詞同時搜尋、判斷、製圖與排版。
...
3. `audit-news-candidates`：保存候選決定與理由、D／E 內部分級及持續事件比較；無法跨次保存時降級為本輪稽核。
4. `verify-news-events`：多來源搜尋、原始／官方資料回查、主張比對與不確定性。
5. `build-news-maps`：判斷空間意義並產生自製定位地圖。
6. `collect-news-images`：取得官方資訊圖與新聞配圖，下載／截圖並視覺驗收。
7. `recover-news-run`：偵測失敗或中斷，只調度失敗事件與模組重跑並重新驗證。
8. 套用 `news-brief-template.md`，再執行 `scripts/validate_news_brief.py`。
