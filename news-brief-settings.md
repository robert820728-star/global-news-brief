...
保存每日新聞簡報的編輯偏好、板塊、分級與收納標準。執行流程、來源複查、地圖、圖片、自主恢復及欄位所有權由 repo 內六個技能負責，避免單一提示詞同時搜尋、判斷、製圖與排版。
...
3. `verify-news-events`：多來源搜尋、原始／官方資料回查、主張比對與不確定性。
4. `build-news-maps`：判斷空間意義並產生自製定位地圖。
5. `collect-news-images`：取得官方資訊圖與新聞配圖，下載／截圖並視覺驗收。
6. `recover-news-run`：偵測失敗或中斷，只調度失敗事件與模組重跑並重新驗證。
7. 套用 `news-brief-template.md`，再執行 `scripts/validate_news_brief.py`。
...
- 詳報欄位順序固定為時間、來源、地圖（可選）、圖片（可選）、事件細節、各方說法（可選）、分析。
...
- 地圖與圖片各自保留；地圖不計入圖片張數，也不得互相取代。
- 事件資料中已驗收的每張地圖與圖片，都實際出現在讀者版且附有圖說。
