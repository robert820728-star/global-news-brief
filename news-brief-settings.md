...
- 欄位名稱與欄位內容必須在同一段落，例如 `**時間：** 8/12。`、`**來源：** 路透社、官方公告。`
...
逐條詳報排版必須手機閱讀優先：詳報標題必須使用 `### T1. 事件名稱 - A`，不得使用 `標題：T1...`。同一則新聞內，`時間`、`來源`、`事件細節`、`各方說法`、`分析` 的欄位名必須用粗體，且欄位名與內容同一段，例如 `**時間：** 8/12。`。不同欄位之間空一行，形成手機上可見的段落區隔。不得把欄位名與內容拆成兩段，也不得寫成沒有粗體欄位名的 `時間：內容`、`來源：內容`。若省略 `各方說法`，`事件細節` 與 `分析` 之間仍須保留一個空行。`今日總覽` 表格仍維持緊湊，不要在表格列之間插入空行。
...
- `逐條詳報` 欄位名與內容必須同段，例如 `**時間：** 8/12。`；欄位之間空一行，避免手機閱讀過擠或過散。
- `各方說法` 無用時應省略。
