...
- 站內前 30 使用 `public_value_v1` 百分制：公共影響 30、地理／人口範圍 20、急迫與安全 15、結構／政策意義 15、實質新進展 10、核心板塊關聯 10。每筆保存分數與理由；同分依發布時間、原始證據直接性及穩定網址排序。
...
- 十站合併後先按底層事件跨站、跨語言去重；去重後每個候選都必須評為 `SS` 至 `E` 並保存獨立的 `grade_reason`。
