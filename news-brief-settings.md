...
- 圖片只從本則已引用的可信來源取得，不為了補圖額外搜尋未引用來源的圖片。
- 若引用來源本身提供可信照片、戰爭畫面、災害現場圖、颱風路徑圖、衛星圖、官方圖表或資料視覺化，應優先嘗試下載圖片或截圖成可在對話中直接顯示的附件。
- 直接貼原站圖片網址不是優先方案；只有在下載或截圖不可行時，才退回使用原始來源連結。
- 優先使用原始長網址或來源頁網址，不使用短網址、追蹤跳轉網址或不透明 CDN 連結作為主要圖片來源。
- 若來源沒有附圖，或圖片無法可信取得，省略圖片欄，不需寫「無圖片」。
- 多張圖片不得重疊、不得使用圖廊式壓縮排版。必須依序直向排列：圖一圖片、圖一說明（圖一來源）、圖二圖片、圖二說明（圖二來源）、圖三圖片、圖三說明（圖三來源）。
- 圖片只輔助理解，不得因為圖片震撼、漂亮或容易傳播而提高事件等級。
...
- `逐條詳報` 中，依事件分析卡擴寫每個事件，並使用標準條目結構；若本則已引用來源有可信圖片且能下載或截圖，加入可選圖片欄。
- `逐條詳報` 的欄位排版必須手機友善：欄位名稱與欄位內容同一段，不要拆成兩段；不同欄位之間空一行，形成清楚區隔。
...
- `逐條詳報` 每則新聞已使用 `**時間：**內容` 這種欄位名與內容同段格式；不同欄位之間有一個空行，且沒有把欄位名與內容拆成兩段。若有圖片欄，已使用下載或截圖附件優先的直向排列格式。
...
- 欄位名稱與欄位內容必須在同一段落，且欄位標籤後直接接內容，例如 `**時間：**8/12。`、`**來源：**路透社、官方公告。`
- 不得把欄位名稱與欄位內容拆成兩段，例如不得寫成 `**時間：**` 後空一行再寫 `8/12。`
- 欄位與欄位之間必須空一行，讓手機閱讀時能分辨段落。
- 每則新聞的建議格式：
...
**來源：**[來源一](https://example.com)、[來源二](https://example.com)。
...
2. 時間
3. 來源
4. 圖片，可選，只有在引用來源本身提供可信圖片且能穩定顯示時才列
5. 事件細節
6. 各方說法
7. 分析
...
逐條詳報排版必須手機閱讀優先：詳報標題必須使用 `### T1. 事件名稱 - A`，不得使用 `標題：T1...`。同一則新聞內，`時間`、`來源`、可選的 `圖片`、`事件細節`、`各方說法`、`分析` 的欄位名必須用粗體，且欄位名與內容同一段；欄位標籤後直接接內容，例如 `**時間：**8/12。`。不同欄位之間空一行，形成手機上可見的段落區隔。不得把欄位名與內容拆成兩段，也不得寫成沒有粗體欄位名的 `時間：內容`、`來源：內容`。若省略 `各方說法`，`事件細節` 與 `分析` 之間仍須保留一個空行。`今日總覽` 表格仍維持緊湊，不要在表格列之間插入空行。
...
- `逐條詳報` 已更新為包含時間欄位，欄位順序為：標題、時間、來源、圖片（可選）、事件細節、各方說法、分析。
- `逐條詳報` 欄位名與內容必須同段，且欄位標籤後直接接內容，例如 `**時間：**8/12。`；欄位之間空一行，避免手機閱讀過擠或過散。
- `各方說法` 無用時應省略。
...
- 已新增圖片規則：只用本則引用來源提供的可信圖片，下載或截圖成可顯示附件優先；多張圖片直向排列並逐張附說明與來源。
