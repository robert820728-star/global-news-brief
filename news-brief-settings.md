...
- Regional supplements 全數進模型；GDELT 每列完整保存在 relevance gate，只有符合非相對複合訊號的 `content_hydration` 列進入本輪模型輸入。`lightweight_semantic_review` 列保留為未完成語意審查的 coverage 證據，不得偽裝成 `non_news` 或已評分事件。禁止固定 top-N；跨來源／文章熱度只能佐證事件訊號，不能單獨把大量 GDELT rows 送入模型。准入詞彙必須同時涵蓋科學、科技、資安、醫學等非政治領域，並保留高衝突／重大事件碼的獨立複合路徑，避免以偏政治或災害的窄詞表取代完整召回。
