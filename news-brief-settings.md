...
`REGIONAL_SUPPLEMENT_COMPLETE_MODEL_ADMISSION_GATE`：中央社與中新社等 `regional_supplement` 的所有精確窗內 provisional groups 必須完整出現在模型 `candidate_groups`，不得因沒有 GDELT heat、Google Trends、Google News coverage 或關鍵字命中而省略。`MODEL_INPUT_SIGNAL_GATE`：GDELT archive 的每一列都保存在 source pool 與逐列 relevance gate；只有具絕對交叉佐證，並命中一個高嚴重事件詞或至少兩個公共利益領域詞的列進入本輪模型輸入。事件 root code、Goldstein 幅度與 heat 只能補充已准入的判定，不能獨立建立 `content_hydration` 工作。`lightweight_semantic_review` 不是 `non_news`、不是已評分事件，也不得計入 Reader coverage。這個分流不得使用固定 top-N 或相對名次。模型輸入建立後、語意合併與六項評分前，只有可執行 runtime 時才執行 `python3 scripts/validate_local_source_admission.py --preprocessed <preprocessed-candidates.json> --selection <selection-results.json> --source-pool news-source-pool.json`；`MOBILE_STRUCTURAL_ADMISSION_EQUIVALENT`：mobile-native 必須逐一比對 regional supplements 的 provisional group IDs 與各組 candidate IDs，確認 selection 完整包含且沒有新增未發現列，不符合時不得繼續，也不得宣稱已執行 Python validator。
...
- `INDEPENDENT_VISIBLE_MEDIA_CAPABILITY_PROBE`：`page_open`、`native_image_search`、`webpage_region_screenshot`、`source_media_byte_fetch` 與 `local_attachment_media_handoff` 分別實測；任一成功不得推導另一項可用。
- `VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE`：只有 `source_media_byte_fetch`（或真正可用的 `webpage_region_screenshot`）、解碼／尺寸／雜湊驗證與 `local_attachment_media_handoff` 全部通過時才優先本機附件；有 runtime／可寫檔案系統本身不足以強制此路徑。
- `MOBILE_NATIVE_IMAGE_SEARCH_FALLBACK_GATE`：完整 `VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE` 不成立時，使用本輪已實測可用的原生圖片卡或其他媒體輸出；若卡片被序列化為一般文字中的 `image_group`／`image_ref`，立即判該路徑失敗。只有 `webpage_region_screenshot` 已獨立實測成功時才改用頁面圖片區域截圖，否則依既有來源順序繼續下一個已驗證路徑。禁止等待或重複輸出 ref。
...
- `CURRENT_EVENT_CONTEXT_PHOTO_GATE`：可靠媒體在同一篇當期事件報導中使用、且可核對為同日同地的現場脈絡照片可作為合格當期新聞圖片，不要求直接拍到核心行動發生的一瞬間。搜尋須使用精確文章標題、來源名稱及可取得的 caption／攝影者／地點；圖說不得描述成直接拍攝核心行動。其他年份、其他地點、歷史檔案或泛用示意圖仍不得入選。
- `RELATIVE_MEDIA_URL_RESOLUTION_GATE`：文章的 `img`、`srcset`、`og:image` 或圖集若提供協定相對、根目錄相對或路徑相對位址，必須以重新導向後的文章 URL 為基準解析；有效 `base href` 優先套用，再產生絕對 HTTP(S) URL。不得把相對路徑本身當成下載失敗或來源無圖。
- `SAME_OCCURRENCE_NATIVE_IMAGE_REF_GATE`：Scheduled Task 的原生 `image_ref` 必須由目前 occurrence 的實際圖片工具結果建立，並在同一最終訊息中真正渲染成 image group／圖片卡。前一個 task、其他對話或其他 occurrence 的 ref 文字不可重用；只列 ref id、聲稱已建立、或詢問「如果你看得到」都算未交付。
- `NON_TEXT_MEDIA_CONTENT_BLOCK_GATE`：一般 assistant 文字中的 `image_group`、`async_image_group`、`image_ref`、`turn...image...` 或 JSON 都不是圖片附件。Scheduled Task 只有在宿主建立非文字 image/media content block 並實際呈現非零尺寸像素時才算交付；結構化讀回若只有 `agentMessage text`，必須判定未交付，禁止圖片 PASS 或 canonical completed。
- `FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：依獨立 capability probe 選可完成的端到端路徑；完整 local pipeline 才走 bytes／真正截圖→驗證→附件，否則使用已驗證的原生媒體路徑。`page_open` 不得推導 `webpage_region_screenshot`；單一路徑失敗不得停止。
...
- Regional supplements 全數進模型；GDELT 每列完整保存在 relevance gate，只有符合非相對複合訊號的 `content_hydration` 列進入本輪模型輸入。`lightweight_semantic_review` 列保留為未完成語意審查的 coverage 證據，不得偽裝成 `non_news` 或已評分事件。禁止固定 top-N；跨來源／文章熱度、事件 root code 與 Goldstein 幅度都只能補充已由文字相關性與絕對交叉佐證准入的判定，不能單獨把大量 GDELT rows 送入模型。准入詞彙必須同時涵蓋科學、科技、資安、醫學等非政治領域，避免以偏政治或災害的窄詞表取代完整召回。
