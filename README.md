...
若要在一般手機 ChatGPT 對話執行，不使用手機 Codex，請先選擇 `Instant` 並貼上 [mobile-chatgpt-start-prompt.md](mobile-chatgpt-start-prompt.md)。此低消耗版本不執行本機程式、地圖或圖表，但仍保留十四天海選清單、每筆六項大評分與所有 C 級以上新聞的讀者版。圖片維持原本選圖，優先使用同圖的 640px 小尺寸版本；無法縮小時允許使用同一張原圖，原圖也無法可靠內嵌時直接提供圖片說明，不用圖片網址或原網站連結代替圖片。
