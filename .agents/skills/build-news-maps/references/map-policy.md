...
- 點位必須直接標示地名，禁止以 `1`、`2`、`3` 等純數字代替。
- 地名必須使用輸出語言；環境缺少相應字型時必須修復字型或 renderer，不得降級成英文或數字。
- 圖說只說明地點與事件的關係、影響範圍或路線意義，不重複「這是世界地圖」「完整行政界線底圖」等顯而易見資訊。
- `map.assets[].place_labels` 必須與圖面實際文字一致，供發布閘門確認。
- `map.assets[].place_labels` 必須符合 `run.language`。繁體中文輸出時，每個地名標籤都必須包含繁體中文，不得只保留英文或當地語言；字型問題屬製圖失敗，必須恢復後重跑。
## 每則事件都要有判定
...
自訂板塊必須輸出淡黃色陸地、清楚深色行政界線的 `maps/generated/sections/<CODE>-base.png`／`.svg`。單一國家必須以 ISO 三碼透過 `scripts/fetch_admin_boundaries.py` 從指定的 canonical 資料入口取得 ADM1；不論選擇美國、英國、法國或其他國家皆使用同一流程，禁止硬編碼國家特例。跨國區域至少保留成員國國界。ADM1 無法取得時必須失敗並恢復，不得降級成無行政區底圖；不得用圖片搜尋結果或官方專業資訊圖冒充板塊底圖。
