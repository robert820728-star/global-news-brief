...
每則事件另須設定 `map.claim_critical`。只有地圖或空間圖層本身直接證明核心主張時才為 `true`；一般定位輔助為 `false`。`required=true` 但非主張關鍵的地圖若 renderer 失敗，可改為 `omitted`，保存內部 `omission_reason` 與 `reader_omission_note` 後繼續文字 reader；讀者版不得留下地圖說明占位，`claim_critical=true` 仍必須 `ready`。
...
- 先把每則需要地圖的事件寫成一份小型 overlay JSON：`section` 使用 manifest 的三碼板塊代碼；`TWN`、`CHN`、`GLB` 使用內建 canonical basemap，自訂板塊讀取 `maps/generated/sections/<CODE>-base.json`。`output` 是 `maps/generated` 下的相對輸出名；`highlights[].match` 使用來源 GeoJSON 的精確行政區鍵值（台灣縣市用 `county`、中國省區用 `id` 或 `name`、全球國家用 `ISO_A3`），`label` 寫繁體中文，`role` 使用 `primary` 或 `secondary`。以 bundled Python 執行 `scripts/render_base_maps.py --overlay-spec <file>`，不得另寫臨時繪圖程式。
