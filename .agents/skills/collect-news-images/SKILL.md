...
- `SCHEDULED_NATIVE_IMAGE_SEARCH_FIRST_GATE`：Scheduled Task 對每則事件先實際呼叫原生圖片搜尋；一般 web search、文章 open 與圖片 URL 解析不能冒充 image search。先從原引用文章取得精確標題、caption、攝影者、日期與地點並搜尋原引用來源；沒有合格 ref 時，依官方／當事組織、原始通訊社、當地可靠媒體與當地語言、其他可靠媒體逐層更換來源，每次用精確事件／日期／地點及新來源重新搜尋。取得合格 ref 後立即交付；跨來源原生搜尋完成前不得卡進直接 URL／下載或宣告失敗。全部原生搜尋仍無 ref 時才走直接文章媒體與宿主可用截圖。
...
- `FIXED_VISIBLE_IMAGE_TRANSPORT_SEQUENCE`：Scheduled Task 固定先完成原引用來源 → 官方／當事組織 → 原始通訊社 → 當地可靠媒體與當地語言 → 其他可靠媒體的跨來源原生圖片搜尋，合格 `image_ref` 立即放入最終 image group／圖片卡；全部仍無 ref 時才試各來源的直接媒體與宿主實際存在的頁面圖片區域截圖。full-runtime 固定用既有 materializer 下載 bytes；下載失敗立即改用本機 `screenshot_path`，通過既有媒體驗證後交付本機實體附件。單一路徑失敗不得停止。
