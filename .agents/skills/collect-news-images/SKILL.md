...
- `LOCAL_ATTACHMENT_FIRST_WHEN_RUNTIME_AVAILABLE_GATE`：宿主有可寫檔案系統與可執行 runtime 時，逐則先解析原文章／官方／可靠轉載媒體，下載或截圖成實體圖片檔，通過既有 materializer 驗證後以本機附件交付。原生圖片搜尋只可定位候選，不能取代本機附件；已有 ref 也不得跳過下載／截圖。
- `MOBILE_NATIVE_IMAGE_SEARCH_FALLBACK_GATE`：只有無本機能力時才使用原生圖片卡；卡片若只被序列化成一般文字中的 `image_group`／`image_ref`，立即判失敗並改走同一來源頁圖片區域截圖，再依既有跨來源順序繼續。不得乾等、重複輸出 ref 或把候選定位當成交付。
