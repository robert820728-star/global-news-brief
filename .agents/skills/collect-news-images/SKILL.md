...
- `INDEPENDENT_VISIBLE_MEDIA_CAPABILITY_PROBE`：分別實測 `page_open`、`native_image_search`、`webpage_region_screenshot`、`source_media_byte_fetch` 與 `local_attachment_media_handoff`；任一成功不得推導另一項可用。
- `VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE`：只有 `source_media_byte_fetch`（或真正可用的 `webpage_region_screenshot`）、解碼／尺寸／雜湊驗證與 `local_attachment_media_handoff` 全部通過時才優先本機附件。有 runtime／可寫檔案系統本身不足以強制本機路徑；缺一即改走另一條已驗證的原生媒體路徑。
- `MOBILE_NATIVE_IMAGE_SEARCH_FALLBACK_GATE`：完整 `VERIFIED_LOCAL_MEDIA_PIPELINE_ROUTE` 不成立時，使用本輪已實測可用的原生圖片卡或其他媒體輸出；卡片若只被序列化成一般文字中的 `image_group`／`image_ref`，立即判失敗。只有 `webpage_region_screenshot` 已獨立實測成功時才改走同一來源頁圖片區域截圖，否則依既有跨來源順序繼續下一個已驗證路徑。不得乾等、重複輸出 ref 或把候選定位當成交付。
...
- `CURRENT_EVENT_CONTEXT_PHOTO_GATE`：可靠媒體在同一篇當期事件報導中使用、且拍攝或發布資訊可核對為同日同地的現場脈絡照片，屬合格的當期新聞圖片；不要求直接拍到核心行動發生的一瞬間。用精確文章標題、來源名稱及可取得的 caption／攝影者／地點重新做原生圖片搜尋。Reader 圖說必須忠實描述畫面，不得描述成直接拍攝核心行動。其他年份、其他地點、歷史檔案或泛用示意圖仍不得入選。
...
- `NON_TEXT_MEDIA_CONTENT_BLOCK_GATE`：禁止把 `image_group`、`async_image_group`、`image_ref`、`turn...image...` 或 JSON 寫進一般 assistant 文字後當成原生圖片交付。Scheduled Task 必須產生真正非文字 image/media content block 並呈現非零尺寸像素；若外部讀回只有 `agentMessage text`，必須判定圖片未交付，禁止圖片 PASS 或 canonical completed。
