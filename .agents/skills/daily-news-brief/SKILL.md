...
- 所有入選事件不分評級都必須逐一檢查每個引用來源頁的官方／媒體圖片，並保存可驗證頁面證據；C+、C、C- 不得跳過。
- 優先取得官方或專業資訊圖，再保留互補的新聞現場照或來源配圖。
- 下載或截圖為可直接顯示的本地附件，完成視覺、時間與內容驗收。
- 將合格附件寫入 `images`。
- 依事件內容判定官方專業圖資需求；地震、疫情、漏油、海洋污染、氣象、災害、戰爭與航運等類型不得使用事件編號白名單或評級門檻規避。
...
- 三組附件路徑必須兩兩獨立；任何一組完成都不得取代、關閉或跳過另外兩組。
- 繁體中文輸出時，地圖上的每個具體地名及 `place_labels` 都必須使用繁體中文；只有英文的標籤視為 `build-news-maps` 失敗。
- 自製圖表不得包含純文字摘要卡，也不得讓圖片階段跳過來源頁檢查。
- 單一可靠來源事件有來源限制文字，但等級未被自動改變。
- 所有附件都有路徑、圖說、來源、驗收狀態；地圖不計入圖片數量。
- 所有地圖都保留完整板塊畫布：台灣完整台灣、中國完整中國、世界完整世界；任何裁切、局部放大或語言錯誤一律由 `recover-news-run` 退回 `build-news-maps` 重做，通過後自動續行發布。
- 所有入選事件必須完成全部引用來源頁的圖片檢查並保存本地證據。任何來源已找到可用圖片但缺少合格附件時，維持 `recovering`，退回 `collect-news-images` 重做；修復後重新驗證並自動續行發布。
