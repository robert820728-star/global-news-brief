...
3. `preprocess-news-candidates`：只做時間窗、URL 正規化、完全重複與初步聚類，不做選稿或評級。
   ...
   - `EVENT_REGION_AND_TIME_IDENTITY_GATE`：要求 `select-news-events` 在六項評分前從內容填妥 `event_identity.country_codes`、`primary_country_code`、`location_evidence`、`event_occurred_at`、`material_update_at`、`material_update_type` 與 `material_update_evidence`。來源分桶不是事件地區，文章發布時間不是事件發生或實質更新時間。舊事件只是重新整理、回顧、週年、換標題或重刊時必須記為 `non_news`；缺漏或矛盾維持 `unresolved`，不得進評分、選入或 reader。
