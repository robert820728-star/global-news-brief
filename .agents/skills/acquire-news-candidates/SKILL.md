...
接著以 `scripts/validate_source_scan_evidence.py` 驗證實際成功的 discovery scans，並以 `news-source-candidate-list.schema.json` 及 `news-relevance-gate.schema.json` 驗證兩份候選清單與 gate。Gate 的 decisions 必須逐列守恆，且所有 regional supplements 都進 admitted output。至少一個 configured discovery source 取得可核實候選即可交給 `select-news-events`；same-source 搜尋備援只能恢復該 configured route。所有 configured routes 都沒有可核實候選時才停止。
