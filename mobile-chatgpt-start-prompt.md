...
`VISIBLE_MEDIA_SCHEDULE_ELIGIBILITY_GATE`／`EVERY_DAILY_NEWS_EXECUTION_GATE`：manual, single-run, test, first-run, recurring, or resume 使用相同新聞與逐則可見圖片門檻。建立或修正排程時先更新最新版完整 task prompt，再在建立或更新排程的目前對話直接截圖公開頁面的圖片區域或交付原生圖片卡；這不是 Scheduled Task occurrence，不要求立即觸發指定 task ID，也不得等待 occurrence 執行介面。不得要求先取得 verified workspace、台灣底圖、原始檔或原畫質。
...
`NON_TEXT_SMOKE_OUTPUT_GATE`：smoke 回覆本身必須含真正可見的非文字 image/media content block 或附件。只有字面 `!:chatgpt-content-reference{...}`、`image_ref`、Markdown、URL、圖說或路徑，或可觀測結果為 `attachments=[]` 時，不得宣稱 smoke 通過；必須保持同一 task 暫停並以原生圖片卡或公開頁面圖片區域直接截圖重試。
...
prompt 驗證後，依 INSTALL.md 在建立或更新排程的目前對話直接執行可見圖片 smoke，使用原生圖片卡或公開頁面圖片區域的直接截圖，確認目前對話實際可見。這不是 Scheduled Task occurrence，不要求立即觸發指定 task ID，也不得因沒有 occurrence 立即執行介面而停止；通過才啟用循環並回覆排程已完成，失敗則保留最新版 prompt、暫停同一 task 並回報實際失敗步驟。

smoke 的同一則回覆必須真正含非文字圖片／附件；字面 `!:chatgpt-content-reference{...}`、`image_ref`、Markdown、URL、圖說、路徑或 `attachments=[]` 都是 FAIL，禁止回覆 smoke 已通過。
```
